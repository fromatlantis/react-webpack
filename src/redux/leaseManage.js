...
const model = {
    ...
    state: {
        ...
        searchParamsrenterRecord: {
            //租赁管理-租赁房源(详情)-租客记录List参数
            pageNo: 1,
            pageSize: 10,
        },
        searchParamsRentalLists: {
            //可出租房源列表List的参数
            pageNo: 1,
            pageSize: 10,
        },
        searchParamsEnterApplyLists: {
            //企业入驻申请列表查询
            pageNo: 1,
            pageSize: 10,
        },
    ...
    actions: [
        ...
        },
        {
            ...
            *effect(action) {
                const res = yield call(request, {
                    type: 'get',
                    url: `/asset/housePublish?houseId=${action.payload.houseId}&status=${
        ...
        },
        {
            ...
            reducer: (state, action) => {
                return {
                    ...state,
                    searchParamsrenterRecord: {
                        ...state.searchParamsrenterRecord,
                        ...action.payload,
                    },
                }
            },
            *effect(action) {
                const params = yield select(
                    rootState => rootState.leaseManage.searchParamsrenterRecord,
                )
                const res = yield call(request, {
                    type: 'post',
        ...
        },
        {
            ...
            *effect(action) {
                const params = yield select(
                    rootState => rootState.leaseManage.searchParamsRentalLists,
                )
                const res = yield call(request, {
                    type: 'post',
        ...
        },
        {
            ...
            reducer: (state, action) => {
                return {
                    ...state,
                    searchParamsEnterApplyLists: {
                        ...state.searchParamsEnterApplyLists,
                        ...action.payload,
                    },
                }
            },
            *effect(action) {
                const params = yield select(
                    rootState => rootState.leaseManage.searchParamsEnterApplyLists,
                )
                const res = yield call(request, {
                    type: 'post',
